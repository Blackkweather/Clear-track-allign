{{-- Zone d'upload photo en glisser-déposer (PPT slide 72 : « Glissez la photo ici ou parcourir »)
     ...
     `required` : affiche l'astérisque rouge des champs obligatoires, comme le
     reste des formulaires, et pose l'attribut HTML correspondant. Les formulaires
     portent `novalidate` : c'est bien la validation serveur qui fait foi, et
     l'attribut ne sert qu'aux technologies d'assistance. --}}
@props(['name', 'label', 'type' => null, 'accept' => 'image/jpeg,image/png', 'required' => false])
    ...
    <span class="text-sm font-medium text-slate-700">
        ...
        @if ($guide)
            <button type="button" @click="exemple = true"
                    class="text-xs text-ppt-blue underline hover:text-brand-800 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500">voir exemple</button>
        @endif
    </span>
    <label
        ...
        <input type="file" name="{{ $name }}" accept="{{ $accept }}" class="sr-only" x-ref="input"
               @required($required) @change="fichier = $event.target.files[0]?.name ?? null">
        <span x-show="!fichier">Glissez la photo ici ou <span class="font-semibold text-ppt-blue underline">parcourir</span></span>
        <span x-show="fichier" x-cloak class="flex items-center gap-2 font-medium text-ppt-blue">
        ...
        <div x-show="exemple" x-cloak @keydown.escape.window="exemple = false"
            ...
            <div @click.outside="exemple = false" x-transition.opacity
                ...
                <div class="flex items-start justify-between gap-4">
                    <h3 id="exemple-titre-{{ $name }}" class="text-lg font-bold text-ppt-blue">{{ $guide['titre'] }}</h3>
