...
@section('content')
    ...
    {{-- LES 5 QUALITÉS (PPT slide 3) : photo annotée avec lignes de rappel --}}
    ...
    <section class="bg-waves" aria-labelledby="experts-titre">
        <div class="mx-auto grid max-w-7xl items-center gap-10 px-4 py-16 sm:px-6 md:grid-cols-2">
            <div class="text-white">
                <h2 id="experts-titre" class="text-3xl font-bold md:text-4xl">Traitement fourni par des dentistes experts dans des cabinets et cliniques dentaires</h2>
                <div class="mt-4 h-1 w-16 rounded bg-white/70"></div>
                <p class="mt-6 text-white/90">Votre traitement sera mené, géré et suivi par nos dentistes certifiés dans des cliniques dentaires entièrement équipées afin de garantir un traitement facile et sans douleurs.</p>
    ...
    {{-- CLEARTRACK EN 4 ÉTAPES (PPT slides 5-8) --}}
    <section class="mx-auto max-w-7xl px-4 py-16 sm:px-6" aria-labelledby="etapes-titre">
            ...
            @foreach ($etapes as $etape)
                <article class="card flex flex-col overflow-hidden !p-0">
                    ...
                    <div class="flex flex-1 flex-col p-6">
                        ...
                        <p class="mt-2 text-sm leading-relaxed">{{ $etape['texte'] }}</p>
                    </div>
                </article>
            @endforeach
        </div>
    ...
    {{-- VIDÉO « Comment ça marche » (issue du kit média officiel du client) --}}
    <section class="mx-auto max-w-5xl px-4 py-16 sm:px-6" aria-labelledby="video-titre">
    ...
    {{-- RÉSULTATS (PPT slides 9-10) — carrousel --}}
    <section class="bg-waves" aria-labelledby="resultats-titre">
        <div class="mx-auto max-w-7xl px-4 py-16 sm:px-6">
            <div class="text-center">
                <h2 id="resultats-titre" class="text-3xl font-bold text-white md:text-4xl">Résultats garantis.<br>Sourires transformés.</h2>
                <p class="mt-3 text-white/90">Les résultats sont visibles en 6 mois en moyenne</p>
            </div>
                ...
                <x-carousel label="Résultats de patients" :per-view-md="3">
                    ...
                    @foreach ($resultats as $r)
                        <div class="shrink-0 px-3" data-carousel-slide :style="`width: ${100 / perView}%`">
                            <div class="card h-full overflow-hidden text-left !p-0">
    ...
    {{-- AVANTAGES 3 COLONNES (PPT slide 11) --}}
    <section class="mx-auto max-w-7xl px-4 py-16 sm:px-6" aria-labelledby="avantages-titre">
        ...
        <div class="mt-10 grid gap-8 md:grid-cols-3">
            ...
            @foreach ($blocs as $bloc)
                <div class="text-center">
                    <span class="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-brand-500 p-4">
                        <img src="{{ asset('assets/' . $bloc['icon']) }}" alt="" class="h-10 w-10 object-contain brightness-0 invert" loading="lazy">
                    </span>
                    <h3 class="mt-4 text-lg font-bold text-brand-600">{{ $bloc['titre'] }}</h3>
                    <p class="mt-2 text-sm leading-relaxed">{{ $bloc['texte'] }}</p>
                </div>
            @endforeach
        </div>
        <div class="mt-8 text-center">
    ...
    {{-- NOUS AIMONS VOTRE SOURIRE (PPT slide 12) --}}
    <section class="bg-waves" aria-labelledby="sourire-titre">
        <div class="mx-auto grid max-w-7xl items-center gap-10 px-4 py-16 sm:px-6 md:grid-cols-2">
            ...
            <div class="text-white md:order-1">
                <h2 id="sourire-titre" class="text-3xl font-bold md:text-4xl">Nous aimons votre sourire&nbsp;!</h2>
                <p class="mt-4 text-xl font-semibold">Rendons-le plus beau…</p>
                <ul class="mt-6 space-y-3 text-white/90">
                    <li class="flex gap-3"><span aria-hidden="true">✓</span>Nous éliminons les espaces entre les dents</li>
                    <li class="flex gap-3"><span aria-hidden="true">✓</span>Nous redressons les dents inclinées et retournées</li>
                    <li class="flex gap-3"><span aria-hidden="true">✓</span>Obtenir un alignement parfait des dents</li>
                </ul>
                <p class="mt-6 text-lg font-bold">Votre Sourire est Magnifique&nbsp;!</p>
                <a href="{{ route('cas-traitables') }}" class="btn-white mt-6">Voir les cas que nous pouvons traiter</a>
            </div>
        </div>
    </section>

    {{-- TRAITEMENT INVISIBLE (PPT slide 13) --}}
    <section class="mx-auto grid max-w-7xl items-center gap-10 px-4 py-16 sm:px-6 md:grid-cols-2" aria-labelledby="invisible-titre">
        <div>
            <h2 id="invisible-titre" class="section-title">Un traitement orthodontique invisible…</h2>
            <p class="mt-4 leading-relaxed">Les aligneurs Cleartrack® sont fréquemment prescrits pour corriger divers cas de dents mal alignées, d’espaces entre les dents et de rotation des dents.</p>
            <div class="mt-8 flex flex-col items-start gap-3">
                ...
                <a href="https://wa.me/212693133170" target="_blank" rel="noopener" class="btn-brand">
        ...
        <div class="flex justify-center">
            <img src="{{ asset('assets/photo-sourire-1.jpg') }}" alt="Aligneur invisible posé sur les dents" class="w-full max-w-md rounded-2xl shadow-xl" loading="lazy">
        </div>
    </section>

    {{-- TÉMOIGNAGES (PPT slides 14-15) — carrousel --}}
    <section class="bg-waves" aria-labelledby="temoignages-titre">
            ...
            <div class="px-6 sm:px-10">
                <x-carousel label="Témoignages de patients" :per-view-md="2">
                    ...
                    @foreach ($temoignages as $t)
                        <div class="shrink-0 px-3" data-carousel-slide :style="`width: ${100 / perView}%`">
                            <blockquote class="h-full rounded-2xl border-2 border-white/60 p-8 text-white">
                                <p class="leading-relaxed">« {{ $t['texte'] }} »</p>
                                <footer class="mt-4 font-bold">{{ $t['auteur'] }}</footer>
                            </blockquote>
                        </div>
                    @endforeach
                </x-carousel>
